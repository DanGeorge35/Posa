...
//---------------------------------------------------------------------------------- Admin Authentication function
	 ...
	 */
	public static function auth_admin($email,$pass) { return true; $db=config::dbcon();
...
//---------------------------------------------------------------------------------- Users Authentication function
	 ...
	 */
	 ...
	 $login_token=cf::selany('login_token','property_users','login_email',$email);
	 $password=md5($pass.$login_token);
	 $stmt = $db->prepare("SELECT * FROM property_users WHERE login_email=:email and login_password=:password");
	 $stmt->execute(array(':email' => $email, ':password' => $password));
	 $row_count = $stmt->rowCount();
	 if($row_count>0){ return true; }
	}
...
//---------------------------------------------------------------------------------- Users Login function (SESSION)
	/*
	 * 1. Create a session of the member_id. taken from the users table using the provided users email
	 ...
	 */
	 ...
	 $_SESSION['member_id']=cf::selany('member_id','property_users','login_email',$email);
	 $_SESSION['member_login_token']=cf::selany('login_token','property_users','login_email',$email);
	 cf::log_login($email);
	 cf::redirect($redirect);
	}
//------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------- Check Users Token function
	 ...
	 * 2. If not return false back as a result
	 * 3. If so find the login token in the users table  at field member_id using the member_id session 
	...
	public static function checkauthuser() {
	 ...
	 { return false;}
	 else{ $login_token=cf::selany('login_token','property_users','member_id',$_SESSION['member_id']);
...
//---------------------------------------------------------------------------------- Get the  member_id session function
...
//---------------------------------------------------------------------------------- Change user role
	...
	*/
	...
	cf::update('property_users','role',$newrole,'id',$id);
	}
//------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------- Check if user's informaton is availble or not
	...
	*/
	public static function checkuserinfo($userid) {
		$count=cf::countrow('id','user_info','member_id',$userid);
...
//---------------------------------------------------------------------------------- Display the users information using the field provided 
	public static function display_user($col) { 
		$info=cf::selany($col,'property_users','id',$_SESSION['user']);
		echo $info;
	}

//---------------------------------------------------------------------------------- Display the admin information using the field provided 
	public static function display_admin($col) { 
		$info=cf::selany($col,'property_users','id',$_SESSION['admin']);
		echo $info;
	}	
...
//---------------------------------------------------------------------------------- Update the fields in a table with the data provided using 1 field comparison
	...
	public static function update_profile($set,$val,$member_id = ''){  $db=config::dbcon();
	 ...
	 $stmt = $db->prepare("UPDATE property_users SET ".$set."=? WHERE member_id=?");
...
public static function timestamp_to_date($timestamp) {
	...
	//---------------------------------------------------------------------------------- Check time_elapsed_string
	 

public static function time_chat_elapsed($datetime) {
		$today = new DateTime(); 
...
public static  function time_elapsed_string($ptime,$type=1)
     {
     	if($type==1){
...
*/
	public static function validate($type,$val) {
	 ...
	 if($type=='newemail') {
		if(cf::countrow('login_email','property_users','login_email',$val) > 0)
		{return false;} else  { return true;}
		}
	 if($type=='newusername') {
		if(cf::countrow('username','property_users','username',$val) > 0)
		{return false;} else  { return true;}
		}
	 if($type=='reff_exist') {
		if(cf::countrow('user_ref','property_users','user_ref',$val) > 0)
...
public static function get_member_image($member_id){
  ...
  $agents= mysqli_fetch_array($result);
	if (empty($agents['propic'])) { 
			...
			$image = SITE_URL.'/images/user9p.png'; }
			else { $image = $agents['fb_img']; }
		} 
	else { $image = SITE_URL."/".$agents['propic'];  } 
	return $image; 
}
...
public static function attack_detect($offence,$mem_id){
		...
		$sql = " insert into db_offence_tb (ip_address ,date_time ,ip_info,offence,member_id) values ('".get_client_ip()."','". date("F j, Y, D, h:i a")."','".$ip_info."','$offence','$mem_id')";
...
/*
CONVERT  SIZE IN BYTE TO DIFFERENT SIZES
-----------------------------------------------------------------------*/
