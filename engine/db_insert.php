...
public static function providers( $provider, $charge_type, $charge_code){
     $db=config::dbcon();
     $stmt = $db->prepare('INSERT INTO providers(`provider`,`charge_type`,`charge_code`) VALUES(?,?,?)');
     $stmt->execute(array($provider,$charge_type,$charge_code));
}
//EXAMPLE// dbi::providers($provider, $charge_type, $charge_code);



public static function ref_linker( $phone, $ref_code){
     $db=config::dbcon();
     $stmt = $db->prepare('INSERT INTO ref_linker(`phone`,`ref_code`) VALUES(?,?)');
     $stmt->execute(array($phone,$ref_code));
}
//EXAMPLE// dbi::ref_linker($phone, $ref_code);


public static function wallet( $user_id, $type, $amount, $status,$details){
     $db=config::dbcon();
     $stmt = $db->prepare('INSERT INTO wallet(`user_id`,`type`,`amount`,`status`,`details`) VALUES(?,?,?,?,?)');
     $stmt->execute(array($user_id,$type,$amount,$status,$details));
}
//EXAMPLE// dbi::wallet($user_id, $type, $amount, $status);



public static function payment_prove( $ticket_id, $amount, $user_id, $package_id, $duration, $img_src, $dated, $status){
     $db=config::dbcon();
     $stmt = $db->prepare('INSERT INTO payment_prove(`ticket_id`,`amount`,`user_id`,`package_id`,`duration`,`img_src`,`dated`,`status`) VALUES(?,?,?,?,?,?,?,?)');
     $stmt->execute(array($ticket_id,$amount,$user_id,$package_id,$duration,$img_src,$dated,$status));
}
//EXAMPLE// dbi::payment_prove($ticket_id, $amount, $user_id, $package_id, $duration, $img_src, $dated, $status);


public static function advert( $adv_id, $img, $link, $type, $views, $click, $publish){
     $db=config::dbcon();
     $stmt = $db->prepare('INSERT INTO advert(`adv_id`,`img`,`link`,`type`,`views`,`click`,`publish`) VALUES(?,?,?,?,?,?,?)');
     $stmt->execute(array($adv_id,$img,$link,$type,$views,$click,$publish));
}
//EXAMPLE// dbi::advert($adv_id, $img, $link, $type, $views, $click, $publish);
