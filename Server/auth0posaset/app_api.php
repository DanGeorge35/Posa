...
if(!empty($_POST['delete_signal'])) {
    $data = new Data();    extract($_POST); 
    $stmt = $db->query("SELECT * FROM advert WHERE id='$did'");
    $dsignal = $stmt->fetch(PDO::FETCH_ASSOC);
    if(!empty($dsignal)){
       if(!empty($did)){        
            $picture= cf::selany('img','advert','id',$did);
            if(!empty($picture)){
              $ls = cf::countrow('img','advert','img',$picture) ;
                if($ls < 2){
...
if(!empty($_POST['set_publish'])) {
    $data = new Data();    extract($_POST); 
    $stmt = $db->query("SELECT * FROM advert WHERE id='$did'");
    $dsignal = $stmt->fetch(PDO::FETCH_ASSOC);
      
      if(!empty($dsignal)){
        cf::update('advert','publish',$dvalue,'id',$did);
           $data->res= "200";
      }else{
           $data->res= "300";
      }
